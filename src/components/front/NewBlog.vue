<template>
    <div class="whitebg bloglist" >
        ...
        <ul>
            <li v-for="(item,index) in articleTops" :key="index">
    ...
    export default {
        ...
        watch: {
            ...
            search: function(newVal,oldVal){
              ...
              this.pageNo=1
              param.data.title = newVal
                this.truth=true
              this.list(param)
            },
            childevenTitle: function(newVal,oldVal){
               this.pageParam.title=newVal.data.title
            },
        ...
        methods:{
            ...
            list(param){
                const vm = this;
                getPageList(param).then(reponse =>{
                  vm.articles = reponse.data.records
                  vm.pageParam.total = reponse.data.total
                  vm.pageParam.pageNo = reponse.data.current
                }).catch(error=>{
            ...
            searchPage(param){
                search(param).then(reponse=>{
                    ...
                    this.pageParam.pageNo = reponse.data.current
                }).catch(error=>{
                    this.$message.error(error)
                })
            },
